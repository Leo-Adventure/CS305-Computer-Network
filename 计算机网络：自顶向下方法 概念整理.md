...
- 深入（靠近端用户，改善用户感受时延和吞吐量）(**Enter Deep**)
- 邀请做客（将集群放在IXP中）(**Bring Home**)
...
经 HTTP 的动态适应性流（**DASH**）: Dynamic Adaptive Streaming over HTTP
...
加性增乘性减（**AIMD**）: Additive Increase Multiplicative Decrease

明确拥塞通告（**ECN**）：Explicit Congestion Notice

**QUIC**：Quick UDP Internet Connections(应用层协议)
